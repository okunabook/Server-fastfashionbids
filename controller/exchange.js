...
exports.readexchange = async (req, res, next) => {
    try {
        const {id} = req.params 
        db.query(
            "SELECT exchange.exchange_name,exchange.exchange_brand,exchange.exchange_color,exchange.exchange_detail,exchange.exchange_want,exchange.exchange_img,sizes,sex.sexname,type.name as typename,exchange.id_sex,exchange.id_type,exchange.id_size " +
            "FROM exchange " +
            "INNER JOIN users ON exchange.id = users.id " +
            "INNER JOIN sex ON exchange.id_sex = sex.id_sex " +
            "INNER JOIN type ON exchange.id_type = type.id_type " +
